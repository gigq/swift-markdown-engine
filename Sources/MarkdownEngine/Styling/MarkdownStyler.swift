...
extension MarkdownStyler {
    struct StylingContext {
        ...
        let baseDescriptor: PlatformFontDescriptor
        let fontName: String
...
enum MarkdownStyler {
    ...
    static func styleAttributes(
        ...
        let ctx = StylingContext(
            ...
            inlineMarkerFont: PlatformFont.systemFont(ofSize: hiddenMarkerSize),
        ...
        result += styleStrikethroughs(ctx)
...
extension MarkdownStyler {
    ...
    static func shrinkInactiveMarkers(_ ctx: StylingContext) -> [StyledRange] {
        ...
        for (i, token) in ctx.tokens.enumerated() where !ctx.activeTokenIndices.contains(i) {
            ...
            let smallFont = PlatformFont(name: ctx.fontName, size: smallSize) ?? PlatformFont.systemFont(ofSize: smallSize)
